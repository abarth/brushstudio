 ...
 *
 * Everything runs inside one page against the real WebGPU engine, because
 * that is the only place the answers are real: a stub would agree with
 * whatever the tests expected. Cases are grouped so the whole suite is one
 * browser launch.
 *
 *   npm test
 *   npm test -- --verbose
...
const results = await page.evaluate(async () => {
  ...
  const paint = async (settings, samples, size = [400, 200], seed = 5) => {
    const surface = await BS.Surface.create(size[0], size[1]);
    surface.clear();
    surface.paint(settings, samples, { seed });
    return surface.readAlpha();
  };
  ...
  await test('hardness narrows the falloff band', async () => {
    const soft = await BS.measureBrush(BS.makeBrush({ tip: { size: 60, hardness: 0 } }), { seeds: 1 });
    const hard = await BS.measureBrush(BS.makeBrush({ tip: { size: 60, hardness: 1 } }), { seeds: 1 });
    assert(
  ...
  await test('spacing over 100% leaves gaps a stroke cannot fill', async () => {
    const tight = await BS.measureBrush(BS.makeBrush({ tip: { size: 40, spacing: 0.1 } }), { seeds: 1 });
    const loose = await BS.measureBrush(BS.makeBrush({ tip: { size: 40, spacing: 2 } }), { seeds: 1 });
  ...
  await test('a smooth round brush measures as non-repeating', async () => {
    const m = await BS.measureBrush(BS.makeBrush({ tip: { size: 40, spacing: 0.1 } }), { seeds: 1 });
  ...
  await test('a dual mask with gappy spacing shows up as a break', async () => {
    ...
    assert(m.repetition.worstGap > 0.2, `a gappy dual train reported worstGap ${m.repetition.worstGap}`);
  });

  await test('scatter widens the band without filling it', async () => {
    const plain = await BS.measureBrush(BS.makeBrush({ tip: { shape: 'spatter', size: 40 } }), { seeds: 1 });
    const spread = await BS.measureBrush(
      BS.makeBrush({
  ...
  await test('a document is a patch: omitted fields keep their defaults', async () => {
    ...
    assert(resolved.settings.tip.spacing === BS.defaultBrush().tip.spacing, 'spacing was not defaulted');
  });
  ...
  await test('a plate renders every row it was asked for', async () => {
    const plate = await BS.plate([doc({ tip: { size: 30 } })], {}, {
      width: 500,
      strokes: ['flat', 'taper'],
    });
    assert(plate.rows.length === 2, `plate had ${plate.rows.length} rows`);
    assert(plate.png.startsWith('data:image/png'), 'the plate was not a PNG');
  ...
  await test('a scattered brush gets a row tall enough to hold it', async () => {
    const tight = await BS.plate([doc({ tip: { size: 30 } })], {}, { width: 400, strokes: ['flat'] });
    const wide = await BS.plate(
      [doc({ tip: { size: 30 }, scatter: { enabled: true, scatter: 3, bothAxes: true } })],
      {},
      { width: 400, strokes: ['flat'] },
    );
    assert(wide.height > tight.height * 1.5, `${wide.height} was not much taller than ${tight.height}`);
  });
